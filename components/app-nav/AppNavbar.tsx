...
import {
  ...
  ChevronRight, Shield, HelpCircle, Star, Zap,
...
const NAV_SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Profit Analytics", href: "/dashboard",              icon: CircleDollarSign },
...
function UserAvatar({ name, size = "sm" }: { name: string; size?: "sm" | "md" | "lg" }) {
  ...
  const colors = ["from-emerald-400 to-emerald-600", "from-blue-400 to-blue-600", "from-violet-400 to-violet-600", "from-pink-400 to-rose-600"];
  const grad = colors[name.charCodeAt(0) % colors.length];
  const sz = size === "lg" ? "h-14 w-14 text-xl" : size === "md" ? "h-10 w-10 text-sm" : "h-8 w-8 text-xs";
  return (
    <div className={`flex shrink-0 items-center justify-center rounded-full bg-gradient-to-br ${grad} text-white font-bold shadow-sm ${sz}`}>
      {initials}
    </div>
  );
}
    ...
    <>
      ...
      <header className="sticky top-0 z-30 flex h-14 items-center border-b border-slate-200/80 bg-white/95 backdrop-blur-md px-4 shadow-sm">
        ...
        <button
          ...
          className="flex h-9 w-9 items-center justify-center rounded-xl text-slate-500 hover:bg-slate-100 transition-colors mr-3"
        >
          <Menu size={20} />
        </button>
        ...
        <Link href="/dashboard" className="flex items-center gap-2 mr-4">
          ...
          <span className="text-sm font-extrabold text-slate-900 tracking-tight hidden sm:block">Arthmount</span>
        </Link>
        ...
        <Link href="/dashboard/wallet"
          ...
          {userLoading
            ? <span className="h-3.5 w-14 animate-pulse rounded bg-emerald-200 inline-block" />
            : <span className="text-xs font-bold text-emerald-700 tabular-nums">{fmtBalance(walletBalance)}</span>
          }
        </Link>
        ...
        <button type="button"
          ...
          className="relative flex h-9 w-9 items-center justify-center rounded-xl text-slate-500 hover:bg-slate-100 transition-colors mr-1">
          <Bell size={18} />
          ...
          <div className="mx-4 mt-4 rounded-2xl bg-gradient-to-br from-[#141720] to-slate-700 p-4 text-white">
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-bold truncate group-hover:text-emerald-300 transition-colors">{user.name}</p>
            ...
            <div className="mt-3 flex items-center justify-between border-t border-white/10 pt-3">
              ...
              >
                <ArrowDownCircle size={12} /> Add Money
              </Link>
            </div>
          </div>
        )}
