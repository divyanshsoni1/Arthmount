...
import {
  ...
  Loader2, Plus, TrendingDown, TrendingUp,
  Wallet, BarChart3, Activity, CircleDollarSign,
} from "lucide-react";
import {
...
import {
  ...
  type PortfolioSummary, type Investment,
              ...
              return (
                <tr key={inv.id} className="border-b border-slate-50 hover:bg-slate-50/40 transition-colors">
                  <td className="px-4 py-3">
                    <p className="font-semibold text-slate-900 text-sm">{inv.packageName}</p>
                  ...
                  <td className="px-4 py-3 font-semibold text-slate-800 tabular-nums">
                    {fmtINR(inv.principalAmount)}
                  </td>
                  <td className="px-4 py-3">
                    <p className="font-semibold text-emerald-600 tabular-nums">+{fmtINR(inv.totalProfitEarned)}</p>
                    {inv.pendingProfit > 0 && (
                      <p className="text-[11px] text-amber-600">{fmtINR(inv.pendingProfit)} pending</p>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[11px] text-slate-500 shrink-0">{progress}%</span>
                    </div>
                    <p className="text-[11px] text-slate-400 mt-0.5">{inv.completedDays}/{inv.tenureDays} days</p>
                  </td>
...
function ActivityFeed({ entries }: { entries: { id: string; transactionType: string; entryType: string; amount: number; description: string; createdAt: string }[] }) {
  if (!entries.length) {
    return (
  ...
  return (
    <div className="space-y-0">
      {entries.map((e, i) => {
        ...
        return (
          <div key={e.id} className={`flex items-center gap-3 px-5 py-3 hover:bg-slate-50/50 transition-colors ${i < entries.length - 1 ? "border-b border-slate-50" : ""}`}>
...
function ProfitChart({ data, loading }: { data: { date: string; profit: number }[]; loading: boolean }) {
    ...
    <div className="rounded-2xl border border-slate-100 bg-white shadow-sm p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Daily credited profit</p>
        </div>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <AreaChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
            ...
            <Tooltip
              ...
              formatter={(v) => [fmtINR(Number(v)), "Profit"]}
            />
...
function KycBanner({ status }: { status: string }) {
  ...
  const cfg: Record<string, { msg: string; cls: string; href: string; btn: string }> = {
    PENDING:    { msg: "Complete KYC verification to unlock full investment features.", cls: "bg-amber-50 border-amber-200 text-amber-800", href: "/dashboard/kyc",  btn: "Start KYC"       },
    IN_REVIEW:  { msg: "Your KYC is under review. We'll notify you within 24–48 hours.", cls: "bg-blue-50 border-blue-200 text-blue-800",   href: "/dashboard/kyc",  btn: "View Status"     },
    REJECTED:   { msg: "Your KYC was rejected. Please re-upload your documents.",        cls: "bg-red-50 border-red-200 text-red-800",       href: "/dashboard/kyc",  btn: "Update Documents"},
  };
  const c = cfg[status];
  if (!c) return null;
  return (
    <div className={`flex items-center justify-between gap-3 rounded-2xl border px-4 py-3 ${c.cls}`}>
      ...
      <Link href={c.href} className="shrink-0 rounded-xl border border-current/20 bg-white/60 px-3 py-1.5 text-xs font-bold hover:bg-white/80 transition-colors whitespace-nowrap">
        {c.btn}
      </Link>
    </div>
  );
...
export default function DashboardPage() {
  ...
  const kpiCards: KpiProps[] = [
    ...
    { label: "Today's Profit",        value: fmtINR(s?.todayProfit    ?? 0),        icon: ArrowDownLeft,    gradient: "bg-gradient-to-br from-amber-500 to-amber-600",         trend: s?.todayProfit ?? 0 > 0 ? "up" : "neutral" },
    ...
    { label: "Overall ROI",           value: `${s?.roi ?? "0.00"}%`,               icon: TrendingUp,       gradient: "bg-gradient-to-br from-indigo-500 to-indigo-700",      trend: "up"      },
    { label: "Active Investments",    value: String(s?.activeInvestments    ?? 0),  icon: BadgeCheck,       gradient: "bg-gradient-to-br from-cyan-500 to-cyan-700",           trend: "neutral" },
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {/* Header */}
      <div className="sticky top-0 z-10 border-b border-slate-200/60 bg-white/95 backdrop-blur-md px-4 sm:px-6 py-3">
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <Link
              ...
              className="flex items-center gap-1.5 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-bold text-white shadow-sm shadow-emerald-600/25 hover:bg-emerald-700 transition-colors"
            >
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: "Active Plans",      value: s?.activeInvestments    ?? 0, icon: TrendingUp       },
            { label: "Completed Plans",   value: s?.completedInvestments ?? 0, icon: BadgeCheck       },
            { label: "Pending Withdrawals", value: s?.pendingWithdrawals ?? 0, icon: Clock            },
            { label: "Total Transactions", value: s?.totalInvestments   ?? 0, icon: Activity         },
        ...
        {s && s.recentDeposits.length > 0 && (
          <div className="rounded-2xl border border-slate-100 bg-white shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              ...
              <Link href="/dashboard/wallet" className="text-xs text-primary font-semibold hover:underline flex items-center gap-1">
